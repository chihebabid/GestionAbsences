...
void PrinterManager::imprimerAbsenceSeance() {
    ...
    const auto& seance = model->getSeance()->getSelectedSession();
    QString heureFin {am::calculateEndTime(seance.start,seance.duration.toInt())};
    ...
    auto printHeader = [&]() {
        ...
        m_painter.setFont(normalFont);
        m_painter.drawText(marginLeft + 70, y + normalMetrics.ascent(), seance.date + " de " + seance.start + " à " + heureFin);
        y += 2 * lineHeight;
...
void PrinterManager::imprimerSynthese() {
    SummaryTableModel * model = qobject_cast<SummaryTableModel*>(m_model);
    if (!model) {
        qDebug() << "Erreur de conversion dynamique...";
        return;
    }
    ...
    const auto& liste = model->getStudentList();
    ...
    auto printHeader = [&]() {
        ...
        m_painter.drawText(marginLeft, y + normalMetrics.ascent(), "Module : ");
        m_painter.setFont(normalFont);
        m_painter.drawText(marginLeft + 70, y + normalMetrics.ascent(), _info.module+" ( "+model->getCourseTypeList()+ " )" );
        y += lineHeight;

        drawFormattedText(marginLeft + 70, y + normalMetrics.ascent(),"%B%N%B%N", QString::number(model->nbSessions()), " séance(s) "
                                                                                                                        "effectuée(s) / "
                                                                                                                        "",
                                                                                                                        QString::number
                                                                                                                        (model->nbTotalSessions()), " séance(s) planifiée(s)");
        y += 2 *lineHeight;
    ...
    for (int i {}; i < liste.size(); ++i) {
        ...
        const auto& e = liste[i];
        QStringList values = {e.inscri, e.lastName, e.firstName, QString::number(e.nbAbsences),QString::number(e.nbAbsences*100.0f/model->nbSessions(),'f',2)+" \%"};
        int x = marginLeft;
        for (int j = 0; j < values.size(); ++j) {
            QRect cellRect(x, y, colWidths[j], lineHeight);
            m_painter.drawRect(cellRect);
